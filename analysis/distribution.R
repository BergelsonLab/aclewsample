...
mated <- ggplot(df, aes(x=age_mo_round)) +
            geom_histogram(aes(fill=mated), binwidth=1)
mated
...
gender <- ggplot(df, aes(x=age_mo_round)) +
            geom_histogram(aes(fill=child_sex), binwidth=1)
gender
...
corpus_dist <- ggplot(df, aes(x=age_mo_round)) +
                geom_histogram(aes(fill=corpus), binwidth=1)
corpus_dist
ggsave("sample_fill_bycorpus.pdf", device='pdf')

total_gender <- ggplot(df, aes(child_sex)) +
                  geom_bar(stat='count', aes(fill=child_sex))
total_gender
